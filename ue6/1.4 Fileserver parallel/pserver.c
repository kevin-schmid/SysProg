...
// WNOHANG prüft, ob die Kindprozesse wirklich beendet
// und nicht ev. noch blockiert sind 
static void no_zombie (int signr) {
  pid_t pid;
  int ret;
  while ((pid = waitpid (-1, &ret, WNOHANG)) > 0)
    printf ("Child-Prozess mit pid=%d hat sich beendet\n",
       pid );
  return;
}
...
int main (void) {
  ...
  while(1) {
	...
	/* Pfad ggf konfigurieren */
       strcpy( path_file, getenv("HOME"));
       strcat( path_file, "/Schreibtisch"); 
